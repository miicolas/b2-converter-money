...
const CURRENCIES = [ // Liste des devises disponibles
...
CURRENCIES.forEach((currency) => { // Ajout des options dans les inputs
...
amount.addEventListener("input", async (event) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`HTTP erreur! statut: ${response.status}`);
